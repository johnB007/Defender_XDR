...
| # | Tile | Purpose |
|---|---|---|
| 1 | **Installed Browser Extensions: Rare or Known-Bad** | Authoritative inventory of every extension installed across Chrome, Edge, Brave, and Firefox. Filters out approved business extensions and flags any match against publicly known IOCs. **Use this tile** to answer "what's installed in my fleet that I don't recognize?" |
| 2 | **Known-Bad and Rare .crx Drops (with extension IDs)** | Catches every Chrome and Edge install, update, and policy push as it lands on disk. Flags drops where the writing process is not a browser or the destination is outside a real browser profile. **Use this tile** to catch supply-chain pushes, malicious sideloads, and policy-deployed extensions. |
| 3 | **Suspicious No-ID .crx Drops (dropper plus path scored)** | Catches non-canonical `.crx` files: bundleware payloads, renamed installers, and partial writes. Legitimate enterprise tooling like Power Automate Desktop is recognized and scored to zero so it stays visible without creating noise. **Use this tile** to catch bundleware-style drops that don't fit the normal install pattern. |
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FjohnB007%2FDefender_XDR%2Fmain%2FWorkbooks%2FBrowser-Extensions-Hunt-Workbook%2Fazuredeploy.json)

[![Deploy to Azure Gov](https://aka.ms/deploytoazuregovernbutton)](https://portal.azure.us/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FjohnB007%2FDefender_XDR%2Fmain%2FWorkbooks%2FBrowser-Extensions-Hunt-Workbook%2Fazuredeploy.json)
